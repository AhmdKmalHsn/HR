...
where cast(Datetime as date) >= @f and cast(Datetime as date) <= @t ";
        ...
        public JsonResult SalaryFileNo(string fileNo, int M, int Y)//AMMAR only//+Ahmed Eid
        {
            #region query
...
select EmployeeId,count(*)cnt,DateFrom
 ...
 left join ( 
 select Date,Amount
 from Payments where EmployeeId=(select id from Employees where FileNumber=@fno) and PaymentDeductionId=1
 )py1 on py1.Date =cte.dateDay left join ( 
 select Date,Amount
 from Payments where EmployeeId=(select id from Employees where FileNumber=@fno) and PaymentDeductionId=2
 )py2 on py2.Date =cte.dateDay left join ( 
 select Date,Amount
 from Payments where EmployeeId=(select id from Employees where FileNumber=@fno) and PaymentDeductionId=4
 )py4 on py4.Date =cte.dateDay left join ( 
 select Date,Amount
 from Payments where EmployeeId=(select id from Employees where FileNumber=@fno) and PaymentDeductionId=5
 )py5 on py5.Date =cte.dateDay left join ( 
 select Date,Amount
 from Payments where EmployeeId=(select id from Employees where FileNumber=@fno) and PaymentDeductionId not in(1,2,4,5)
 )py0 on py0.Date =cte.dateDay 
